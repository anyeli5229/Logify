...
export default function TaskCard({ task, canEdit }: TaskCardProps) {
    ...
    return (
        <li 
            ref={setNodeRef}
            style={style}
            className={`p-3.5 bg-white rounded-xl border border-slate-200/80 shadow-xs hover:shadow-md transition-shadow duration-200 group relative flex items-start justify-between gap-2 select-none ${
                isDragging ? 'z-50 shadow-xl ring-2 ring-indigo-500/50 opacity-50' : ''
            }`}
        >
            {/* Zona arrastrable (Listeners aplicados solo al contenido de la tarea) */}
            <div 
                {...listeners} 
                {...attributes} 
            ...
            >
                <h4
                    onClick={(e) => {
                        e.stopPropagation();
                        navigate(location.pathname + `?viewTask=${task.id}`);
                    }}
            ...
            {/* Menú aislado de los eventos de arrastre */}
            <Menu as="div" className="relative shrink-0 z-10">
                ...
                <MenuItems 
                    transition
                    className="absolute right-0 z-20 mt-1 w-40 origin-top-right rounded-xl bg-white p-1 shadow-lg ring-1 ring-slate-900/5 transition duration-100 ease-out data-closed:scale-95 data-closed:opacity-0 text-xs font-medium focus:outline-none"
                >
                    <MenuItem>
                        <button
                            type="button"
